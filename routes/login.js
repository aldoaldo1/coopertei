...
var DB, Everyone, Auth;

var Login = function(db, everyone) {
  DB = db;
  Everyone = everyone;
...
Login.get = function(req, res, next) {
  ...
  var _ua = uaParser.parse(req.headers['user-agent']);

  if (_ua.family !== 'Chrome' && _ua.family !== 'Chromium') {
    res.send(404);
  } else if (isMobile()) {
		res.send(404);
		return;
    ...
    res.render('login', { layout: false, msg: 0 });
  }
};

Login.post = function(req, res, next) {
  Auth.doLogin(req.body.user, req.body.pass, function(user) {
    if (user) {
      ...
      req.session.destroy(function(err) {
        res.render('login', { layout: false, msg: 1 });
      });
    }
  });
};
